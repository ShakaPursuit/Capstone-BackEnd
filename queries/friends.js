...
const getFriendRequests = async () => {
  try {
    const showAllFriendConnection = await db.any("SELECT * FROM connection_requests");
...
const getSingleFriend = async (connectionrequests_id, receiver_user_profile_id) => {
    try {
...
const createFriends = async (request) => {
    try {
    const { sender_user_profile_id, receiver_user_profile_id, status } = request;
...
const deleteFriendRequest = async (connectionrequests_id, sender_user_profile_id, receiver_user_profile_id, status, timestamp) => {
    try {
      const deletedFriend = await db.one(
        ...
        [connectionrequests_id, sender_user_profile_id, receiver_user_profile_id, status, timestamp]
      );
...
module.exports = { createFriends, getFriendRequests, deleteFriendRequest, getSingleFriend };
